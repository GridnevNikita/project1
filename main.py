...
@patch('requests.get')
def test_get_weather(moke_get):
    moke_get.return_value.json.return_value = {'main': {'temp': 1}}
    assert get_weather(1, 1) == 1
    moke_get.assert_called_once_with(f'https://api.openweathermap.org/data/2.5/weather?lat=1&lon=1&appid={API_KEY}')
